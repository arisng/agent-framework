...
For the higher-level topology interpretation and the transferable AGUIDojo lessons, pair this note with [copilot-cli-session-topology.md](copilot-cli-session-topology.md). The useful takeaway for AGUIDojo is inspectable session surfaces, not literal copying of Copilot CLI's local layout or exact schema.
